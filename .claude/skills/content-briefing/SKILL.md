...
**Note on audit trail:** `briefing-data.json` is generated by `seo-pipeline assemble-briefing-data`. For a complete audit trail, the pipeline should be run with:
```bash
...
5. **Run** the merge script to patch the qualitative fields into `briefing-data.json`:
   ```
   uv run seo-pipeline merge-qualitative --dir $OUT/
   ```
